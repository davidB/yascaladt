...
ids : ID (',' ID)*
  ;
stableId :
    (ID | (ID '.')? 'this' . ID | (ID '.')? 'super' classQualifier? '.' ID) ('.' ID)*
  ;
classQualifier :
    LBRACK ID RBRACK
  ;
...
annotType   : simpleType annotation*
  ;
simpleType  :
    (stableId | stableId '.' 'type' | LPAREN types ','? RPAREN) (typeArgs | '#' ID)*
  ;
typeArgs :
    LBRACK types RBRACK
  ;
...
simpleExpr  :
  ...
  | blockExpr
  | (literal | stableId | '_' | LPAREN (exprs ','?)? RPAREN | simpleExpr '.' ID | simpleExpr typeArgs) (argumentExprs)* '_'?
  ;
  //xmlExpo
exprs :
    expr (',' expr)*
