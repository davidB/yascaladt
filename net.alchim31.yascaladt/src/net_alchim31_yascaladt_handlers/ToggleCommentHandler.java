package net_alchim31_yascaladt_handlers;
...
public class ToggleCommentHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		Log.logger.info( ts.getStartLine() + " " + ts.getEndLine() );
		IEditorPart editorPart = HandlerUtil.getActiveEditor(event);
		if( !( editorPart instanceof TextEditor ) && !(editorPart instanceof MultiPageEditorPart )){
			Log.logger.warning( "Editor is not a TextEditor. Should always be so for the Toogle Comment action" );
			return null;
		}
		...
		if( editorPart instanceof MultiPageEditorPart ){
	        ...
	        for( IEditorPart editor : editors ) {
	            if( editor instanceof TextEditor ){
	                ...
	                Log.logger.info("Found text editor for the MultiPageEditorPart");
	                break;
	            }
	        }
	        
	        if( null == textEditor){
	            Log.logger.warning("The MultiPageEditorPart did not have a TextEditor. Cannot toggle comments");
	            return null;
	        }
		} else {
	...
	public static boolean addComment ( IDocument document, TextSelection selection ) {
		...
		try {
			for( int lineNum = startLine; lineNum <= endLine; lineNum++ ) {
				...
				Log.logger.info( "Checking line " + lineNum + " with text " + lineText );
			...
			Log.logger.warning("BadLocationException when trying to toggle comments\n" + ex );
			return false;
		}
	...
	public static boolean startsWithComment( String line ){
		...
		Log.logger.info( "Match result was " + match + " for '" + line + "'" );
		return match;
